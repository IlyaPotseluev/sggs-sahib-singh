...
def _menu_summary(
    ...
    problem_angs = len(missing) + len(empty) + len(gaps) + len(mm) + len(dup_angs)
    ...
    for num, key, count, extra in rows:
        ...
        marker = "  " if (
            (key == "missing" and not missing) or
            (key == "empty" and not empty) or
            (key == "coverage_gap" and not gaps) or
            (key == "duplicate" and not duplicates) or
            (key == "gurmukhi_mismatch" and not mm)
        ) else "! "
        print(f"    [{num}] {marker}{label:<40}  {count} {extra}")
    print()
...
def run_menu(
    ...
    while True:
        ...
        print()
        try:
            choice = input("  Выбери категорию [1-5 / v / q]: ").strip().lower()
...
def main() -> None:
    ...
    reports = validate_range(start, end, banidb_path, verbose=args.verbose)
    duplicates = find_cross_ang_duplicates(start, end)
    ...
    if args.plain:
        ...
        run_menu(reports, duplicates, start, end, banidb_path=banidb_path)
